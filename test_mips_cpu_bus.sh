...
if [ -z "$INSTRUCTION" ]; then
	...
	for t in $TESTS; do
		...
		### assemble input file
		test/assembler/bin/parser test/tests/$hexed.asm.txt test/hex1/$hexed.hex.txt

		###compile testbench
		iverilog -g 2012 \
			${SOURCEDIR}/mips_cpu.v test/mips_cpu_bus_tb_delay1.v test/RAM_32x2048_delay1.v \ -s mips_cpu_bus_tb_delay1 \ -Pmips_cpu_bus_tb_delay1.RAM_INIT_FILE=\"test/hex1/$hexed.hex.txt\" \ -o test/simulator1/mips_cpu_bus_tb_delay1_$hexed
